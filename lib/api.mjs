...
export const BASE_URL = (process.env.SHOPIFY_ADMIN_BASE_URL || "https://api.shopify-admin.test").replace(/\/$/, "");
...
export async function apiRequest({ method, path, query, body, headers = {}, dryRun, verbose, idempotencyKey, ifMatch, file, version = "0.0.0" }) {
  ...
  const reqHeaders = { "user-agent": `shopify-admin-cli/${version}`, ...headers };
  const auth = applyAuth(reqHeaders);
  if (!auth.ok) errorOut("auth_missing", `Set SHOPIFY_ADMIN_API_KEY (or run 'shopify-admin-cli login') to authenticate.`);
