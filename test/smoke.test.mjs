// Structural tests for shopify-admin-cli — no network, no auth, no surprises.
...
test("unknown resource → validation_error", async () => {
  const r = await runJson(["nope", "list"], { env: { SHOPIFY_ADMIN_API_KEY: "test" } });
  assert.equal(r.exitCode, 1);
  assert.equal(r.errJson.code, "validation_error");
});

test("unknown action → validation_error listing available", async () => {
  const r = await runJson(["items", "frobnicate"], { env: { SHOPIFY_ADMIN_API_KEY: "test" } });
  assert.equal(r.exitCode, 1);
  assert.equal(r.errJson.code, "validation_error");
...
test("required flag missing → validation_error (no crash)", async () => {
  const r = await runJson(["items", "get"], { env: { SHOPIFY_ADMIN_API_KEY: "test" } });
  assert.equal(r.exitCode, 1);
  assert.equal(r.errJson.code, "validation_error");
...
test("--dry-run does not make network requests", async () => {
  const r = await runJson(["items", "list", "--dry-run"], {
    env: { SHOPIFY_ADMIN_API_KEY: "test", SHOPIFY_ADMIN_BASE_URL: "http://127.0.0.1:1" },
  });
  assert.equal(r.exitCode, 0);
