...
const ENV = { SHOPIFY_ADMIN_API_KEY: "test-token" };
...
test("items list returns array", async () => {
  await withMock({
    ...
    const r = await runJson(["items", "list"], { env: { ...ENV, SHOPIFY_ADMIN_BASE_URL: server.url } });
...
test("items list --all walks cursor pages and concatenates", async () => {
  await withMock({
    ...
    const r = await runJson(["items", "list", "--all"], { env: { ...ENV, SHOPIFY_ADMIN_BASE_URL: server.url } });
    assert.equal(r.exitCode, 0, r.stderr);
...
test("items get interpolates id into path", async () => {
  await withMock({
    ...
    const r = await runJson(["items", "get", "--id", "42"], { env: { ...ENV, SHOPIFY_ADMIN_BASE_URL: server.url } });
    assert.equal(r.exitCode, 0, r.stderr);
...
test("items delete returns null body", async () => {
  await withMock({
    ...
    const r = await runJson(["items", "delete", "--id", "9"], { env: { ...ENV, SHOPIFY_ADMIN_BASE_URL: server.url } });
    assert.equal(r.exitCode, 0, r.stderr);
...
test("item-variants list interpolates parent id", async () => {
  await withMock({
    ...
    const r = await runJson(["item-variants", "list", "--itemId", "100"], { env: { ...ENV, SHOPIFY_ADMIN_BASE_URL: server.url } });
    assert.equal(r.exitCode, 0, r.stderr);
...
test("orders list filters by status query", async () => {
  await withMock({
    ...
    const r = await runJson(["orders", "list", "--status", "pending"], { env: { ...ENV, SHOPIFY_ADMIN_BASE_URL: server.url } });
    assert.equal(r.exitCode, 0, r.stderr);
...
test("orders upload posts multipart/form-data with --file", async () => {
  const tmp = mkdtempSync(join(tmpdir(), "shopify-admin-mp-"));
  ...
  try {
    await withMock({
      ...
      const r = await runJson(["orders", "upload", "--id", "ord-1", "--file", filePath], { env: { ...ENV, SHOPIFY_ADMIN_BASE_URL: server.url } });
      assert.equal(r.exitCode, 0, r.stderr);
...
test("404 → not_found", async () => {
  await withMock({ "GET /items/:id": { status: 404, body: { message: "nope" } } }, async (server) => {
    const r = await runJson(["items", "get", "--id", "999"], { env: { ...ENV, SHOPIFY_ADMIN_BASE_URL: server.url } });
...
test("422 → validation_error with details", async () => {
  await withMock({ "POST /items": { status: 422, body: { message: "bad", errors: ["name required"] } } }, async (server) => {
    const r = await runJson(["items", "create", "--name", ""], { env: { ...ENV, SHOPIFY_ADMIN_BASE_URL: server.url } });
    assert.equal(r.exitCode, 1);
...
test("network down → network_error", async () => {
  ...
  await server.close();
  const r = await runJson(["items", "list"], { env: { ...ENV, SHOPIFY_ADMIN_BASE_URL: url } });
  assert.equal(r.exitCode, 1);
...
test("user-agent header is sent", async () => {
  await withMock({ "GET /items": { status: 200, body: { items: [], nextCursor: null } } }, async (server) => {
    await runJson(["items", "list"], { env: { ...ENV, SHOPIFY_ADMIN_BASE_URL: server.url } });
    assert.match(server.requests[0].headers["user-agent"], /shopify-admin-cli\//);
  });
});
