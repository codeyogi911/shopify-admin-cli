...
// outside the resource registry and is dispatched directly by bin/shopify-admin-cli.mjs.
//
// Mirrors the agents-cli pattern: `<cli> login [--token <t>]` stores a token at
// ~/.config/shopify-admin-cli/credentials.json. `<cli> login --status` reports
...
export async function runLogin(values, jsonRequested) {
  ...
  if (!token) {
    errorOut("validation_error", "Pass --token <value>, set SHOPIFY_ADMIN_LOGIN_TOKEN, or set SHOPIFY_ADMIN_API_KEY in your environment.");
  }
