...
description: Quick reference for every Shopify Admin API resource × action exposed by shopify-admin-cli. Use when the user asks "what does the items API support?" or needs the precise flag set for a specific endpoint.
allowed-tools:
  - Bash
  - Read
---
...
Resource × action quick reference. When in doubt, prefer `shopify-admin-cli <resource> <action> --help` — the help text is generated from the same registry and never drifts.
